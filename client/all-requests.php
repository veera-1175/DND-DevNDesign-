<?php
    ...
    $sql = "SELECT j.title, f.id_freelancer, f.first_name_free, f.last_name_free, f.profession, f.statut, f.exentation, s.message_job, s.status, s.time_sent, s.id_request, j.id_job
            FROM suivi_job s
            JOIN jobs j ON s.id_job = j.id_job
            JOIN freelancer f ON s.id_freelancer = f.id_freelancer
            WHERE s.status = 'pending' AND j.id_client = $id_client";
    ...
    if (isset($_POST['accept'])) {
        ...
        // Set status to accepted for this request
        $update_sql = "UPDATE suivi_job SET status = 'accepted', time_accepted = NOW() WHERE id_request = $id_request";
        mysqli_query($conn, $update_sql);

        // Set status to rejected for all other requests with the same job id
        $job_id = $_POST['job_id'];
        $update_sql = "UPDATE suivi_job SET status = 'rejected' WHERE id_request != $id_request AND id_job = $job_id";
        mysqli_query($conn, $update_sql);

        // Redirect to requests-accepted.php
        header('Location: requests-accepted.php');
        exit();
    }
    ...
    if (isset($_POST['reject'])) {
        ...
        // Set status to rejected for this request
        $update_sql = "UPDATE suivi_job SET status = 'rejected' WHERE id_request = $id_request";
        mysqli_query($conn, $update_sql);
                ...
                <div class="request_details">
                    <div class="top_detail">
                        ...
                        <div class="more_detail">
                            <span>Request sent on <?php echo date('d-m-Y', strtotime($row['time_sent'])) . ' at ' . date('h:i A', strtotime($row['time_sent'])); ?></span>
                        </div>
                    </div>
                    ...
                    <div class="request_action">
                        ...
                        <form method="POST">
                            ...
                            <button class="btt" type="submit" name="accept"><i class="fa-solid fa-check"></i></button>
                            <button class="btt" type="submit" name="reject" onclick="window.location.reload()"><i class="fa-solid fa-xmark"></i></button>
                        </form>
                    </div>
                </div>
            </div>
